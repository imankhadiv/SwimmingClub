class Notification < ActiveRecord::Base
  ...
  def self.remove_roles_notification (user_id, notifiable_type)
    Notification.where(user_id: user_id, notifiable_type: notifiable_type).destroy_all
  ...
  def self.number_of_notifications_for_roles (user_id)
    (self.check_notification user_id, 'Role').size
  end
